...
from utils.ip_investigation import filter_and_investigate_ips
import gzip
import json
...
async def run(request: AgentRequest, response: AgentResponse, context: AgentContext):
    ...
    try:
        ...
        if email:
            ...
            analysis, slack_delivered = await generate_dmarc_report_from_email(
                email, context
            )
            return response.json(
                {"summary": analysis, "slack_delivered": slack_delivered}
            )
        else:
...
async def generate_dmarc_report_from_email(email: Email, context: AgentContext):
    ...
    for attachment in email.attachments:
        ...
        if not (
            ...
            or attachment.filename.endswith(".xml.gz")
            or attachment.filename.endswith(".zip")
            or "dmarc" in attachment.filename.lower()
        ):
        ...
        try:
            context.logger.info(
                f"Attempting to read attachment data for: {attachment.filename}"
            )
            ...
            try:
                ...
                context.logger.warning(
                    f"Skipping attachment due to validation error: {e}"
                )
                continue
            ...
            if attachment.filename.endswith(".xml"):
                ...
                dmarc_contents.append(xml_content)
                context.logger.info(
                    f"Processed gzipped XML attachment: {attachment.filename}"
                )
                    ...
                    for file_name in zip_file.namelist():
                        if file_name.endswith(".xml"):
                            ...
                            try:
                                ...
                                context.logger.warning(
                                    f"Skipping '{file_name}' from zip: {e}"
                                )
                                continue

                            dmarc_contents.append(xml_content)
                            context.logger.info(
                                f"Extracted XML file '{file_name}' from zip: {attachment.filename}"
                            )

        except ValidationError as e:
            context.logger.warning(
                f"Validation failed for '{attachment.filename}': {e}"
            )
            continue
        except Exception as e:
            error_message = str(e)
            context.logger.error(
                f"Error processing attachment '{attachment.filename}': {error_message}"
            )
            continue
    ...
    # Investigate suspicious IPs across all DMARC reports
    context.logger.info(f"Found {len(dmarc_contents)} DMARC reports to analyze")
    context.logger.info("Starting IP investigation for suspicious IPs...")

    # Combine IP investigation results from all reports
    all_ip_investigations = {}
    excessive_failures_flag = False
    total_failed_ips_count = 0
    ...
    try:
        for xml_content in dmarc_contents:
            ...
            total_failed_ips_count += failed_count

        context.logger.info(
            f"IP investigation completed: {len(all_ip_investigations)} IPs investigated, "
            f"{total_failed_ips_count} total failed IPs, excessive_failures={excessive_failures_flag}"
        )
    except Exception as e:
        context.logger.error(f"Error during IP investigation: {e}", exc_info=True)
        # Continue with DMARC analysis even if IP investigation fails
        all_ip_investigations = {}

    # Format IP investigation results for the prompt
    ip_context = format_ip_investigations_for_prompt(all_ip_investigations)

    # Analyze DMARC reports with IP context
    all_analyses = []
    ...
    # Use IP-enhanced summarization if we have IP context
    if all_ip_investigations:
        summary = await summarize_analysis_with_ip(
            all_analyses,
            email_metadata,
    ...
    # Prepend high-alarm message if excessive failures detected
    if excessive_failures_flag:
        alarm_message = (
        ...
        summary = alarm_message + summary

    # Store analysis results in key-value store
    try:
        await store_dmarc_analysis(
            context, email_metadata, dmarc_contents, all_analyses, summary
        )
        context.logger.info(
            "Successfully stored DMARC analysis results in key-value store"
        )
    except Exception as e:
...
async def store_dmarc_analysis(
    ...
    if dmarc_contents:
        for xml_content in dmarc_contents:
            ...
            if report_metadata and report_metadata.get("report_id"):
                ...
                context.logger.info(
                    f"Using report_id for storage key: {report_metadata.get('report_id')}"
                )
                break
...
def extract_report_metadata(xml_content: str) -> dict:
    ...
    try:
        ...
        result["report_id"] = (
            report_id_elem.text if report_id_elem is not None else None
        )
...
def generate_storage_key(
    report_metadata: dict = None, email_metadata: dict = None
) -> str:
    """
...
def format_ip_investigations_for_prompt(ip_investigations: dict) -> str:
    """
    Format IP investigation results into a readable string for the OpenAI prompt.

    Args:
        ip_investigations: Dictionary mapping IP to IPInvestigationResult

    Returns:
        Formatted string with IP investigation context
    """
    if not ip_investigations:
        return "No IP investigations performed."

    formatted_lines = []
    for ip, result in ip_investigations.items():
        formatted_lines.append(f"\n--- IP: {ip} ---")
        formatted_lines.append(f"Risk Level: {result.risk_level.upper()}")
        formatted_lines.append(f"Recommendation: {result.recommendation.upper()}")
        formatted_lines.append(f"Summary: {result.summary}")
...
async def analyze_dmarc_report_with_ip_context(dmarc_report: str, ip_context: str):
    """
    Analyzes a single DMARC report with IP investigation context using OpenAI GPT.

    This enhanced version includes IP intelligence to provide more actionable
    recommendations (e.g., whether to block, monitor, or ignore failed IPs).

    Args:
        dmarc_report: The DMARC XML report content to be analyzed.
        ip_context: Formatted IP investigation results string.
...
async def summarize_analysis_with_ip(
    ...
    This enhanced version produces actionable Slack notifications that include
    specific recommendations based on IP intelligence.

    Args:
        results: A list of individual DMARC analysis strings (with IP context).
        email: Metadata or identifying information for the email being summarized.
        excessive_failures: Flag indicating unusually high failure rate.
        total_failed_ips: Total number of failed IPs in the report.

    Returns:
        A single summarized report string formatted for Slack.
    """
    if not results:
        summary = "❌ Unable to analyse DMARC report(s) – parsing failed."
        return summary

    template = templates["summarize-analysis-with-ip"]
    compiled_prompt = template.substitute(analysis=results, email=email)
    response = await client.chat.completions.create(
        model=config.OPENAI_MODEL,
        messages=[{"role": "user", "content": compiled_prompt}],
